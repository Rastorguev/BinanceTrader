...
{
    private static readonly ApiClient apiClient = new("@YourApiKey", "@YourApiSecret");
    private static readonly BinanceClient binanceClient = new(apiClient);
    ...
    {
        var candlestick = binanceClient.GetCandleSticks("ethbtc", TimeInterval.Minutes_15, new DateTime(2017, 11, 24),
            new DateTime(2017, 11, 26)).Result;
    }
    ...
    {
        var icebergOrder = binanceClient
            .PostNewOrder("ethbtc", 0.01m, 0m, OrderSide.Buy, OrderType.Market, icebergQty: 2m).Result;
    }
