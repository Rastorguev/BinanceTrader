...
namespace BinanceTrader.Api
{
    ...
    {
        private readonly int _clientServerTimeDiffMs;
        [NotNull] private readonly HttpClient _client = new HttpClient();
        ...
        public BinanceApi([NotNull] IBinanceKeyProvider binanceKeyProvider, int clientServerTimeDiffMs = 0)
        {
            _clientServerTimeDiffMs = clientServerTimeDiffMs;
            var keys = binanceKeyProvider.GetKeys().NotNull();
            _secretKey = keys.SecretKey;
            _client.DefaultRequestHeaders.NotNull().TryAddWithoutValidation("X-MBX-APIKEY", keys.ApiKey);
        ...
        private Uri CreateSignedUri(Uri uri, NameValueCollection queryParams = null)
        {
            ...
            query["timestamp"] = (timestamp + _clientServerTimeDiffMs).ToString();
